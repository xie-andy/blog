...
export const GET: APIRoute = async (context) => {
  ...
  const { results } = await db
    .prepare('SELECT * FROM suggestions WHERE post_slug = ? ORDER BY created_at DESC')
    .bind(slug)
    .all();
...
export const POST: APIRoute = async (context) => {
  ...
  await db
    .prepare('INSERT INTO suggestions (id, post_slug, author_name, content, created_at) VALUES (?, ?, ?, ?, ?)')
